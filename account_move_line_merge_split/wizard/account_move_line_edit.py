...
class AccountMoveLineEdit(models.Model):
    ...
    division_nr = fields.Integer(_('Number of Divisions'), default=2)
    division_line_ids = fields.One2many('account_move_line_edit_line', 'config_id', string=_('Division Lines'), copy=True, auto_join=True)
    move_line_id = fields.Many2one('account.move.line', _('Move Line'))
    move_line_value = fields.Float(_('Total Move Line Value'))
    aggregation_date = fields.Date(_('Aggregation Date'))
    
    def generate_lines(self):
        """
        Generates the division lines for splitting a move line.
        Validates the input values and ensures that the selected move line can be split.
        """
        if not self.division_nr or self.division_nr <= 1:
            raise exceptions.ValidationError(_('You must specify a valid number of divisions.'))
        ...
        if len(move_lines_ids) > 1:
            raise exceptions.ValidationError(_('To divide move lines, you must select only one.'))
        
        move_line_id = self.env['account.move.line'].search([('id', 'in', move_lines_ids)], limit=1)
        self.move_line_id = move_line_id.id
        
        # A move line without a maturity date cannot be divided.
        if not move_line_id.date_maturity:
            raise exceptions.ValidationError(_('Cannot divide the move line "%s" as it does not have a maturity date.') % move_line_id.name)
        ...
        return {
            ...
            'res_id': self.id,
            'view_id': self.env.ref('account_move_line_merge_split.account_move_line_split_form').id,
    ...
    def split_move_line(self):
        ...
        Ensures the sum of the new lines matches the original move line's value.
        """
        if not self.move_line_id or not self.division_line_ids:
            raise exceptions.ValidationError(_('You must generate the division lines first.'))
        
        if self.validate_edit_move_lines(self.move_line_id):
            new_move_line_value = round(sum([line.amount for line in self.division_line_ids]), 2)
            if (self.move_line_id.debit > 0 and not self.move_line_id.debit == new_move_line_value) or \
               (self.move_line_id.credit > 0 and not self.move_line_id.credit == new_move_line_value):
                raise exceptions.ValidationError(_('The sum of the division lines (%s) does not match the original move line\'s value (%s).') % 
                                                  (new_move_line_value, self.move_line_value))
            ...
            for line in self.division_line_ids:
                ...
                if self.move_line_id.credit > 0:
                    new_move_line = self.env['account.move.line'].with_context(check_move_validity=False).\
                        create(self.get_vals(self.move_line_id, line.amount, 0, line.due_date))
                if self.move_line_id.debit > 0:
                    new_move_line = self.env['account.move.line'].with_context(check_move_validity=False).\
                        create(self.get_vals(self.move_line_id, 0, line.amount, line.due_date))
            ...
            if len(new_move_lines) == len(self.division_line_ids):
                ...
                raise exceptions.ValidationError(_('The new move lines could not be generated correctly.'))
    ...
    def merge_move_lines(self):
        ...
        """
        move_lines_ids = self.env.context.get('active_ids', False)
        move_lines = self.env['account.move.line'].search([('id', 'in', move_lines_ids)])
        if len(move_lines) == 1:
        ...
        if self.validate_edit_move_lines(move_lines):
            ...
            merged_move_line = self.env['account.move.line'].with_context(check_move_validity=False).\
                create(self.get_vals(move_lines[0], merged_credit, merged_debit, self.aggregation_date))
    ...
    def validate_edit_move_lines(self, move_line_ids):
        ...
        for move_line_id in move_line_ids:
            if move_id and move_id != move_line_id.move_id.id:
                raise exceptions.ValidationError(_('You have selected move lines from different invoices.'))
            elif account_id and account_id != move_line_id.account_id.id:
                raise exceptions.ValidationError(_('You have selected move lines from different accounts.'))
            else:
                move_id = move_line_id.move_id.id
                account_id = move_line_id.account_id.id
            if move_line_id.reconciled:
                raise exceptions.ValidationError(_('You are trying to edit a reconciled move line.'))
        return True
    ...
    def get_vals(self, move_line_id, credit, debit, date):
        ...
        return {
            ...
            'analytic_account_id': move_line_id.analytic_account_id,
